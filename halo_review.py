...
def build_review_precheck_prompts(payload: dict[str, Any]) -> dict[str, str]:
    ...
    user_prompt = "\n\n".join(
        [
            f"Title: {payload.get('title') or payload.get('source_name')}",
            f"Approximate length: {payload.get('word_count', 0)} words.",
...
def build_review_prompts(persona: str, payload: dict[str, Any], runtime_context: str) -> dict[str, str]:
    ...
    if word_count >= 700:
        length_guidance = "If the material has substance, allow yourself a full review in roughly 260 to 420 words."
    elif word_count >= 180:
        length_guidance = "If the material warrants it, give it a thoughtful review in roughly 180 to 320 words."
    else:
        length_guidance = "Keep the review concise, controlled, and proportionate, roughly 90 to 180 words unless the material clearly earns more."
